...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while (singleQueue.size() + mergeQueue.size() != 1) {
      TreeNode * s = removeSmallest(singleQueue, mergeQueue);
      TreeNode * ss = removeSmallest(singleQueue, mergeQueue);

      unsigned sF = s->freq.getFrequency();
      unsigned ssF = ss->freq.getFrequency();
     
      TreeNode * temp = new TreeNode(sF + ssF);
      temp->left = s;
      temp->right = ss;
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if(bfile.getNextBit()) 
      ...
      temp->left = HuffmanTree::readTree(bfile);
      temp->right = HuffmanTree::readTree(bfile);
      return temp;
    }

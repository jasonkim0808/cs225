...
class KDTree
{
  ...
  private:
     ...
     */
    vector<Point<Dim>> vPoint;
    int getDistance(const Point<Dim> & f, const Point<Dim> & s) const;
    void copy(const KDTree<Dim>& other);
    void destroy(KDTreeNode *s);
    typename KDTree<Dim>::KDTreeNode * makeKDTree(int l, int r, int im);
    void select(int l, int r, int index, int im);
    Point<Dim> FNNHelper(const Point<Dim>& query, KDTreeNode * s, int dim) const;
};
